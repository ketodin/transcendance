...
async function run() {
  ...
  if (!issue) { _log.warn('No issue in payload, skipping'); return; }

  _log.group(`Issue #${issue.number}`);

  const scopeValue = parseScopeFromBody(issue.body);
  if (!scopeValue) { _log.warn('No scope section found in issue body, skipping'); _log.end(); return; }

  const label = SCOPE_MAP[scopeValue];
  if (!label) { _log.warn(`Unknown scope value "${scopeValue}", skipping`); _log.end(); return; }
