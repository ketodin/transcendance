...
function applyGameStart(scene: GameScene, room: Room<GameRoomState>, data: GameStartData) {
...
export async function connectToServer(scene: GameScene) {
	...
	if (pending) {
		...
		applyGameStart(scene, pending.room, pending.gameStartData);
		setupRoomHandlers(scene, pending.room);
		return;
	}
	...
	try {
		...
		room.onMessage('game_start', (data: GameStartData) => {
			applyGameStart(scene, room, data);
		});
