...
<div class="bg-muted/30 flex min-h-screen items-center justify-center px-4">
  <div class="glass w-full max-w-md space-y-6 p-6">
    ...
    <div>
      {#if step === 'credentials'}
        <form onsubmit={e => { e.preventDefault(); void handleSubmit(); }} class="space-y-5">
          <div class="space-y-2">
      ...
      {:else}
        <form onsubmit={e => { e.preventDefault(); void (useBackup ? verifyBackupCode() : verifyTotp()); }} class="space-y-5">
          <p class="text-muted-foreground text-center text-sm">
            {useBackup
          ...
          <div class="flex justify-between text-sm text-muted-foreground">
            <button
              type="button"
              onclick={() => { useBackup = !useBackup; code = ''; error = ''; }}
            >
