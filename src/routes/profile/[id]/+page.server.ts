...
export const actions: Actions = {
	invitePrivateGame: async ({ request, locals }) => {
		...
		if (!userId || typeof userId !== 'string')
			return fail(400, "userId must be a string");

		const room = await matchMaker.createRoom('tank_room', { private: true });
		statusHub.notify(userId, 'invite_request_received', { fromUserId: locals.user!.id, fromUserName: locals.user!.name, roomId: room.roomId });
