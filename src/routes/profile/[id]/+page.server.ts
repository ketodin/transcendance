...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	if (!user)
		return error(404, "User not found"); // maybe redirect instead

	const userType: UserType = locals.user.id === user.id ? 'self'
		: (await isFriend(locals.user.id, user.id) ? 'friend' : 'someone');
